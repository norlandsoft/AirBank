...
/**
 * 现金存款：核心 CASH_DEPOSIT 记账 + 尾箱现金流入（docs/design/03 §4.2）。
 */
@Component
@RequiredArgsConstructor
public class CashDepositStrategy implements BizTypeStrategy {

    private final CoreClient coreClient;

    @Override
    public List<String> bizTypes() {
        return List.of(CounterTxn.BIZ_CASH_DEPOSIT);
    }
    ...
    public Map<String, Object> execute(ExecContext ctx) {
        ...
        Result<TxnVO> r = coreClient.postTxn(null, new TxnCmd(
                ctx.requestNo(), "CASH_DEPOSIT", null, acctNo, ctx.amount(),
        ...
        result.put("acctNo", t.toAcct());
        result.put("amount", t.amount());
        result.put("balanceAfter", t.toBalanceAfter());
